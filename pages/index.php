...
// Sertakan file utilitas dan koneksi
require_once __DIR__ . '/../includes/utils.php';
require_once 'koneksi.php'; // Koneksi database khusus untuk pages

// Variabel global $con dari koneksi.php di /pages
global $con;
...
if (!$con instanceof mysqli) {
    ...
    log_error("Pages Index - Koneksi database tidak valid (\$con).");
}


// --- Konfigurasi Dasar & SEO ---
$nama_toko = "Sedaya Furniture";
$tagline = "Toko Mebel & Furniture Online Berkualitas";
...
$project_folder = "/newsedaya24apr2025"; // <<< PASTIKAN SESUAI
$canonical_url = $base_url . $project_folder . '/pages/index.php'; // URL Halaman ini

// Lokasi Direktori Gambar (URL relatif dari web root)
$url_dir_images_root = $project_folder . "/images";
$url_dir_carrousel = $url_dir_images_root . "/carrousel/";
$url_dir_produk_base = $url_dir_images_root . "/produk/";
$url_dir_galeri = $url_dir_images_root . "/galery/";
...
$fs_dir_galeri = $fs_dir_images_root . '/galery/';

// Placeholder & Default OG Image (URL Absolut)
$default_placeholder_url = $base_url . $url_dir_images_root . "/placeholder.png";
$default_og_image_url = $base_url . $url_dir_images_root . "/og_image_default.png";
...
if (!$database_connection_error) {
    ...
    if ($result_kategori_all) {
        while ($kategori_data = mysqli_fetch_assoc($result_kategori_all)) {
            $categories[] = $kategori_data;
        }
        mysqli_free_result($result_kategori_all);
    } else {
        log_error("Index - Gagal fetch kategori all: " . mysqli_error($con));
    }

    // --- Query untuk Carrousel (Hanya perlu gambar) ---
    $query_carrousel = "SELECT id, gambar, judul FROM carrousel ORDER BY id DESC"; // Ambil judul untuk alt text
    $result_carrousel = mysqli_query($con, $query_carrousel);
    if ($result_carrousel) {
        while ($item = mysqli_fetch_assoc($result_carrousel)) {
            $carrousel_items[] = $item;
        }
        mysqli_free_result($result_carrousel);
    } else {
        log_error("Index - Gagal fetch carrousel: " . mysqli_error($con));
    }
    ...
    if ($stmt_produk) {
        ...
        log_error("Index - Gagal prepare statement produk unggulan: " . $con->error);
    }
    ...
    // --- Ambil Gambar Galeri dari DB ---
     ...
     if ($stmt_galeri) {
        ...
        while ($item_galeri = $result_galeri->fetch_assoc()) {
            $galeri_items_db[] = $item_galeri['gambar'];
        }
        $stmt_galeri->close();
     } else {
         log_error("Index - Gagal prepare statement galeri: " . $con->error);
     }
    ...
    mysqli_close($con);
     ...
     log_error("Index - Tidak dapat menjalankan query karena koneksi database gagal.");
...
if (!empty($carrousel_items) && !empty($carrousel_items[0]['gambar'])) {
    ...
    if (file_exists($fs_path_og)) {
        $og_image_url = rtrim($base_url, '/') . rtrim($url_dir_carrousel, '/') . '/' . $first_carousel_image_name;
    }
}

// Siapkan variabel untuk footer
$footer_categories = array_slice($categories, 0, 6);
$pages_url_base = rtrim($base_url, '/') . $project_folder . '/pages/'; // Diperlukan footer
...
<head>
    ...
    <link rel="icon" href="<?php echo rtrim($base_url, '/'); ?>/images/favicon.png" type="image/x-icon">
    <link rel="apple-touch-icon" href="<?php echo rtrim($base_url, '/'); ?>/images/apple-touch-icon.png">
    ...
    <link rel="stylesheet" href="../assets/css/style.css"> <script type="application/ld+json">
    {
      ...
      "potentialAction": {
        "@type": "SearchAction",
        "target": "<?php echo htmlspecialchars(rtrim($base_url, '/') . $project_folder . '/pages/produk.php?keyword={search_term_string}'); ?>",
    ...
    <script type="application/ld+json">
    {
      ...
      "@type": "FurnitureStore",
      "name": "<?php echo htmlspecialchars($nama_toko); ?>",
      "url": "<?php echo htmlspecialchars($base_url); ?>", // URL utama toko
      "logo": "<?php echo htmlspecialchars(rtrim($base_url, '/') . '/images/logo-sedaya-navbar.png'); ?>",
    ...
    <style>
        body { padding-top: 70px; } /* Padding for fixed navbar */
        /* ... (CSS lainnya dari style.css atau inline jika perlu) ... */
        ...
        .carousel-caption-custom { position: absolute; top: 0; left: 0; right: 0; bottom: 0; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; color: white; z-index: 10; pointer-events: none; /* Agar tidak menghalangi klik kontrol */ }
        ...
        .carousel-caption-custom .btn { pointer-events: auto; /* Aktifkan klik tombol */ }
        ...
        /* ... (CSS lain untuk produk, galeri, dll.) ... */
...
<body>
    <?php
    // Include header dan lewatkan data kategori
       ...
       <div class="carousel-indicators" style="z-index: 11;">
        ...
        <div class="carousel-inner h-100">
            <?php if (!empty($carrousel_items)): ?>
                <?php foreach ($carrousel_items as $index => $item): ?>
                    <?php
                        $gambar_carrousel_url = $default_placeholder_url;
                        ...
                        if (!empty($item['gambar'])) {
                            ...
                            if (file_exists($fs_path)) {
                                $gambar_carrousel_url = rtrim($base_url, '/') . rtrim($url_dir_carrousel, '/') . '/' . $gambar_filename;
                                $alt_carousel = htmlspecialchars($item['judul'] ?: "Sedaya Furniture Banner " . ($index + 1));
                            } else {
                                 log_error("Index - File Carousel tidak ditemukan: " . $fs_path);
                            }
                        }
                    ?>
        ...
        <div class="carousel-caption-custom">
            <h1 class="animate__animated animate__fadeInDown"><?php echo "Selamat Datang di " . htmlspecialchars($nama_toko); // Judul Statis ?></h1>
            <p class="lead animate__animated animate__fadeInUp animate__delay-1s" style="text-shadow: 1px 1px 3px rgba(0,0,0,0.7); max-width: 700px;">
                <?php echo htmlspecialchars($tagline); // Tagline Toko ?>
            </p>
            <a href="produk.php" class="btn btn-primary btn-lg mt-3 animate__animated animate__fadeInUp animate__delay-2s">
                Jelajahi Koleksi Kami
            </a>
        </div>

        <?php if (count($carrousel_items) > 1): ?>
            <button class="carousel-control-prev" type="button" data-bs-target="#heroCarousel" data-bs-slide="prev" style="z-index: 11;">
                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                <span class="visually-hidden">Previous</span>
            </button>
            <button class="carousel-control-next" type="button" data-bs-target="#heroCarousel" data-bs-slide="next" style="z-index: 11;">
    ...
    <section id="welcome" class="py-5">
         <div class="container text-center">
            <h2 class="section-title">Kualitas Terbaik Furniture Jepara</h2>
            <p class="lead text-muted col-lg-8 mx-auto">
                Sedaya Furniture menghadirkan keindahan dan ketahanan mebel asli Jepara. Dibuat oleh pengrajin berpengalaman dengan material kayu pilihan, setiap produk kami adalah investasi untuk kenyamanan dan estetika hunian Anda.
            </p>
             <?php if ($database_connection_error): ?>
                 <div class="alert alert-warning mt-4">Mohon maaf, saat ini kami mengalami gangguan teknis. Beberapa data mungkin tidak dapat ditampilkan.</div>
             <?php endif; ?>
        </div>
    </section>
    ...
    <section id="produk-unggulan" class="py-5 bg-light">
         <div class="container">
                ...
                <?php if (!empty($produk_unggulan)): ?>
                    <?php foreach ($produk_unggulan as $produk): ?>
                        <?php
                            ...
                            $gambar_url = $default_placeholder_url;
                            ...
                            if (!empty($gambar_filename) && !empty($kode_produk)) {
                                ...
                                if (file_exists($fs_path_produk)) {
                                    $gambar_url = rtrim($base_url, '/') . rtrim($url_dir_produk_base, '/') . '/' . $kode_produk . "/" . $gambar_filename;
                                    $alt_produk = htmlspecialchars($produk['nama']);
                                } else {
                                     log_error("Index - File Produk Unggulan tidak ditemukan: " . $fs_path_produk);
                                }
                            }
                        ?>
                        <div class="col d-flex align-items-stretch">
                            <div class="card product-card h-100">
                                <a href="detail-produk.php?id=<?php echo $produk['id']; ?>">
                                     <img src="<?php echo htmlspecialchars($gambar_url); ?>" class="card-img-top" alt="<?php echo $alt_produk; ?>" loading="lazy">
                                </a>
                                <div class="card-body d-flex flex-column">
                                    <h3 class="card-title h6">
                                        <a href="detail-produk.php?id=<?php echo $produk['id']; ?>" class="text-decoration-none text-dark">
                                            <?php echo htmlspecialchars($produk['nama']); ?>
                                        </a>
                                    </h3>
                                    <a href="detail-produk.php?id=<?php echo $produk['id']; ?>" class="btn btn-sm btn-outline-primary details-button mt-auto">Lihat Detail</a>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
            ...
            <div class="text-center mt-5">
                <a href="produk.php" class="btn btn-primary btn-lg">Lihat Semua Produk</a>
            </div>
        </div>
    </section>
        ...
        <div class="row row-cols-2 row-cols-md-3 g-3">
             <?php if (!empty($galeri_items_db)): ?>
                <?php foreach ($galeri_items_db as $index => $gambar_galeri_nama): ?>
                     <?php
                         $gambar_galeri_url = $default_placeholder_url;
                         ...
                         if(!empty($gambar_galeri_nama)) {
                             ...
                             if (file_exists($fs_path_galeri)) {
                                 $gambar_galeri_url = rtrim($base_url, '/') . rtrim($url_dir_galeri, '/') . '/' . $gambar_galeri_nama;
                                 $alt_galeri = "Galeri " . htmlspecialchars(ucwords(str_replace(['-', '_'], ' ', pathinfo($gambar_galeri_nama, PATHINFO_FILENAME))));
                             } else {
                                 log_error("Index - File Galeri tidak ditemukan: " . $fs_path_galeri);
                             }
                         } else {
                              log_error("Index - Nama file Galeri kosong dari DB untuk item index " . $index);
                         }
                     ?>
                    <div class="col">
    ...
    <section id="cta" class="py-5 bg-primary text-white">
         <div class="container text-center">
            ...
            <a href="kontak.php" class="btn btn-light btn-lg me-md-2"><i class="bi bi-envelope-fill me-2"></i>Hubungi Kami</a>
             <a href="https://wa.me/6281567958549?text=Halo%20Sedaya%20Furniture,%20saya%20tertarik%20dengan%20produk%20Anda." target="_blank" class="btn btn-whatsapp btn-lg mt-2 mt-md-0"><i class="bi bi-whatsapp me-2"></i> Chat WhatsApp</a>
        </div>
    </section>

    <?php
    // Include footer dan lewatkan data yang diperlukan
